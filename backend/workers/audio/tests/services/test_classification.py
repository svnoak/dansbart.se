...
class TestClassificationService:
    ...
    def test_save_predictions_rollback_on_error(
        self, classification_service, sample_track, mock_db_session
    ):
    ...
    def test_save_predictions_preserves_user_confirmed_style(
        self, classification_service, sample_track
    ):
        """A user-confirmed style row must survive a reclassify (T1)."""
        ...
        rows = {
            r.dance_style: r
            for r in db.query(TrackDanceStyle)
            .filter(TrackDanceStyle.track_id == sample_track.id)
            .all()
        }
    ...
    def test_save_predictions_skips_style_already_confirmed(
        self, classification_service, sample_track
    ):
        ...
        vals = (
            db.query(TrackDanceStyle)
            .filter(
                TrackDanceStyle.track_id == sample_track.id, TrackDanceStyle.dance_style == "Vals"
            )
            .first()
        )
