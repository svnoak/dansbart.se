...
from app.core.models import Album, Artist, PlaybackLink, Track, TrackAlbum, TrackArtist
...
class TrackRepository:
    ...
    def create_track(
        self, title: str, isrc: str, duration_ms: int, album_data: dict, artists_data: list
    ) -> Track:
        ...
        if album_data and album_data.get("name"):
            ...
            if not existing_album:
                existing_album = (
                    self.db.query(Album)
                    .filter(Album.title == album_data["name"], Album.artist_id == primary_artist.id)
                    .first()
                )
