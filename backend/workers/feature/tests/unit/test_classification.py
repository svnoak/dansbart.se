...
class TestSavePredictionsPreservesConfirmedStyle:
    def test_preserves_user_confirmed_style(self, classification_service, sqlite_session):
        ...
        rows = {
            r.dance_style: r
            for r in sqlite_session.query(TrackDanceStyle)
            .filter(TrackDanceStyle.track_id == track_id)
            .all()
        }
    ...
    def test_skips_style_already_confirmed(self, classification_service, sqlite_session):
        ...
        schottis_rows = (
            sqlite_session.query(TrackDanceStyle)
            .filter(TrackDanceStyle.track_id == track_id, TrackDanceStyle.dance_style == "Schottis")
            .all()
        )
        ...
        vals = (
            sqlite_session.query(TrackDanceStyle)
            .filter(TrackDanceStyle.track_id == track_id, TrackDanceStyle.dance_style == "Vals")
            .first()
        )
