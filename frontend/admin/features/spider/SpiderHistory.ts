                ...
                <table class="w-full text-left">
                    ...
                    <tbody>
                        <tr v-for="log in history" :key="log.id"
                            ...
                            <td class="py-2 px-4 max-w-xs truncate" :title="log.artistName">
                                {{ log.artistName }}
                            </td>
                            ...
                            <td class="py-2 px-4">
                                <span v-if="log.musicGenre"
                                      class="px-2 py-1 bg-indigo-600/30 text-indigo-400 rounded text-xs">
                                    {{ log.musicGenre.replace('_', ' ') }}
                                </span>
                            </td>
                            
                            <td class="py-2 px-4 text-xs text-gray-500 max-w-xs truncate" :title="log.detectedGenres?.join(', ')">
                                {{ log.detectedGenres?.join(', ') || '-' }}
                            </td>
                            
                            <td class="py-2 px-4 text-xs text-gray-500">
                                {{ formatDate(log.crawledAt) }}
                            </td>
