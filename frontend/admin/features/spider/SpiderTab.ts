  ...
  setup() {
    ...
    const loadSpiderHistory = async () => {
      try {
        ...
        const d = data as { content?: unknown[]; items?: unknown[] };
        const items = (d.content ?? d.items ?? []) as unknown[];
        crawlHistory.value = items as any[];
      } catch (e) {
        showError(e.message);
      }
    };
      ...
      const checkStatus = async () => {
        try {
          ...
          if (state === 'PENDING') {
            ...
            const stats = /** @type {{ artists_crawled?: number; tracks_found?: number }} */ (data?.result ?? {});
            spiderMessage.value = `✅ Crawl complete! Found ${stats.artists_crawled ?? 0} new artists, ${stats.tracks_found ?? 0} tracks.`;
            ...
            showToast(
              `Spider complete! ${stats.artists_crawled ?? 0} artists, ${stats.tracks_found ?? 0} tracks`
            );
          } else if (state === 'FAILURE') {
            clearInterval(pollInterval);
            spiderLoading.value = false;
            spiderError.value = true;
            spiderMessage.value = `❌ Crawl failed: ${data?.error ?? 'Unknown'}`;
    ...
    const runSpider = async () => {
      ...
      try {
        ...
        spiderMessage.value = `${data.message} - Task ID: ${data.taskId}`;
    ...
    const handleBlockArtist = async log => {
      ...
      try {
        ...
        showToast(`🚫 Blocked ${log.artistName}`, 'success');
      } catch (e) {
        showError(e.message);
      }
    };
