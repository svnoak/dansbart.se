...
export default {
  ...
  setup() {
    ...
    const rejectArtist = async artistId => {
      ...
      try {
        const data = await rejectApi.confirmRejectArtist(String(artistId), 'Rejected from review tab', true);
        showToast(String(data?.message ?? 'Rejected'), 'success');
        loadData();
      } catch (e) {
    ...
    const blockArtist = async artistId => {
      ...
      try {
        const data = await rejectApi.confirmRejectArtist(String(artistId), 'Blocked from spider crawling', false);
        showToast(String(data?.message ?? 'Blocked'), 'success');
        loadData();
      } catch (e) {
                ...
                <div v-else class="space-y-2">
                    <div v-for="album in pendingAlbums" :key="album.id"
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <div class="text-xs text-gray-400">
                                {{ album.artistName }} • {{ album.pendingCount }} pending
                            </div>
                        </div>
                    </div>
                </div>
