...
export default {
  setup() {
    ...
    const isolatedArtists = computed(() => {
      return artists.value.filter(a => !a.isVerified);
    });
    ...
    const loadData = async () => {
      ...
      try {
        if (view.value === 'isolated') {
          ...
          const d = data as { content?: unknown[]; items?: unknown[]; totalElements?: number; total?: number };
          const items = (d.content ?? d.items ?? []) as unknown[];
          artists.value = items as any[];
          totalArtists.value = Number(d.totalElements ?? d.total ?? 0);
        } else {
          const data = await api.loadBlocklist('artist', limit.value, offset.value);
          const d = data as { content?: unknown[]; items?: unknown[]; totalElements?: number; total?: number };
          const items = (d.content ?? d.items ?? []) as unknown[];
          blocklist.value = items as any[];
          totalBlocklist.value = Number(d.totalElements ?? d.total ?? 0);
        }
      } catch (e) {
    ...
    const approveArtist = async artist => {
      ...
      try {
        ...
        const data = await api.approveArtist(id);
        showToast(String(data?.message ?? 'Approved'));
        loadData();
      } catch (e) {
    ...
    const rejectArtist = async artist => {
      const confirmMsg = `Reject and delete artist "${artist.name}"?\n\n• ${artist.trackCount || artist.pendingCount} tracks will be deleted\n• Artist will be added to blocklist`;

      if (!confirm(confirmMsg)) return;

      try {
        const id = typeof artist.id === 'string' ? artist.id : String(artist.id);
        const data = await api.rejectArtist(id, 'Rejected from quick review');
    ...
    const restoreFromBlocklist = async item => {
      ...
      try {
        ...
        const data = await api.removeFromBlocklist(id);
        showToast(String(data?.message ?? 'Removed from blocklist'));
        loadData();
      } catch (e) {
                ...
                <div v-if="isolatedArtists.length > 0" class="mb-6">
                    ...
                    <div class="space-y-2">
                        <div v-for="artist in isolatedArtists" :key="artist.id"
                            ...
                            <img v-if="artist.imageUrl" :src="artist.imageUrl" class="w-10 h-10 rounded object-cover">
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                <div class="text-xs text-gray-400">
                                    {{ artist.trackCount || artist.pendingCount }} tracks
                                    <span v-if="artist.pendingCount" class="text-yellow-400">({{ artist.pendingCount }} pending)</span>
                                </div>
                        ...
                        <div v-for="artist in nonIsolatedArtists" :key="artist.id"
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                <div class="flex gap-2 mt-1">
                                    ...
                                    <span v-if="artist.sharedTracks > 0" class="text-xs bg-blue-600/20 text-blue-400 px-2 py-0.5 rounded">
                                        🤝 {{ artist.sharedTracks }} collaborations
                                    </span>
                                </div>
                                <div v-if="artist.sharedWithArtists && artist.sharedWithArtists.length > 0" class="text-xs text-gray-500 mt-1">
                                    With: {{ artist.sharedWithArtists.slice(0, 3).join(', ') }}{{ artist.sharedWithArtists.length > 3 ? '...' : '' }}
                                </div>
                            </div>
            ...
            <div v-else-if="view === 'blocklist' && blocklist.length > 0">
                <div class="space-y-2">
                    <div v-for="item in blocklist" :key="item.id"
                        ...
                        <div class="flex-1 min-w-0">
                            <div class="font-bold text-sm truncate">{{ item.entityName }}</div>
                            <div class="text-xs text-gray-400">
                                Blocked on {{ new Date(item.rejectedAt).toLocaleDateString() }}
                            </div>
