  ...
  setup() {
    ...
    const fetchKeywords = async () => {
      ...
      try {
        ...
        keywords.value = items as any[];
        pagination.total = Number(d.totalElements ?? d.total ?? 0);
      } catch (e) {
    ...
    const handleToggleActive = async kw => {
      try {
        ...
        await api.updateKeyword(kw.id, { isActive: kw.isActive });
        showToast(`Keyword ${kw.isActive ? 'activated' : 'deactivated'}`, 'success');
        fetchStats();
      } catch (e) {
        kw.isActive = !kw.isActive; // Revert on fail
    ...
    const openEditForm = kw => {
      ...
      form.mainStyle = kw.mainStyle;
      form.subStyle = kw.subStyle || '';
      form.isActive = kw.isActive;
      formMode.value = 'edit';
      showModal.value = true;
    };
                ...
                <table class="w-full text-left text-sm text-gray-300">
                    ...
                    <tbody class="divide-y divide-gray-700 bg-gray-800">
                        ...
                        <tr v-else v-for="kw in keywords" :key="kw.id" class="hover:bg-gray-750 transition-colors">
                            ...
                            <td class="px-4 py-3 text-gray-400">{{ kw.subStyle || '-' }}</td>
                            <td class="px-4 py-3">
                        ...
                        <form @submit.prevent="submitForm">
                            <div class="space-y-4">
                                ...
                                <div>
                                    ...
                                    <input v-model="form.mainStyle" required placeholder="e.g. 'Polska'"
                                           class="w-full bg-gray-900 border border-gray-600 rounded p-2 text-white focus:border-blue-500 outline-none" />
                                </div>
                                
                                <div>
                                    <label class="block text-xs uppercase text-gray-500 mb-1">Sub Style (Optional)</label>
                                    <input v-model="form.subStyle" placeholder="e.g. 'Slängpolska'"
                                           class="w-full bg-gray-900 border border-gray-600 rounded p-2 text-white focus:border-blue-500 outline-none" />
                                </div>
