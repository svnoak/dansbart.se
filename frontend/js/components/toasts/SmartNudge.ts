...
type TempoCategoryKey = keyof typeof TEMPO_CATEGORY_MAP;
...
export default defineComponent({
  ...
  watch: {
    'track.id': {
      immediate: true,
      handler(newId: string | number | undefined, oldId: string | number | undefined) {
        if (newId !== oldId) {
          this.clearTimers();
          this.step = 'hidden';
  ...
  computed: {
    ...
    hasTempo(): boolean {
      const t = this.track as {
        tempo?: { label?: string };
        tempoCategory?: TempoCategoryKey;
        tempo_category?: TempoCategoryKey;
        effectiveBpm?: number;
    ...
    tempoLabel(): string {
      ...
      const t = this.track as { tempoCategory?: TempoCategoryKey; tempo_category?: TempoCategoryKey };
      const category = t.tempoCategory ?? t.tempo_category;
      return (category && TEMPO_CATEGORY_MAP[category]) || '';
    },
    secondaryTempoLabel() {
    ...
    unconfirmedSecondary(): { style: string; confirmations?: number }[] {
      if (!this.track?.secondary_styles) return [];
      return this.track.secondary_styles.filter((s: { confirmations?: number }) => (s.confirmations ?? 0) < 3);
    },
    colorClasses() {
  ...
  methods: {
    ...
    resetForm() {
      ...
      this.correction.style = this.track?.danceStyle || '';
      this.correction.tempo = 'ok';
    },
    closeDropdownHandler(e: MouseEvent) {
      if (this.dropdownOpen && !(e.target as Element).closest('.relative')) this.dropdownOpen = false;
    },

    // --- SELECTION LOGIC ---

    selectMain(mainStyle: string) {
    ...
    confirmVerify() {
      const specificStyle = this.track.subStyle || this.track.danceStyle;
      this.submit({
        style: specificStyle,
        mainStyle: this.track.danceStyle,
        tempo_correction: 'ok',
      }).then(() => {
    ...
    submitTempoSelection(tempoCategory: TempoCategoryKey) {
      this.submit(
        {
          style: this.correction.style,
          mainStyle: this.correction.main || this.correction.style,
          tempo_correction: 'ok',
          tempo_category: TEMPO_CATEGORY_MAP[tempoCategory] || 'Lagom',
        },
        'success'
      );
    },

    async submit(payload: Record<string, string>, nextState: 'success' | 'bonus' | null = null) {
      this.clearTimers();
            ...
            <div v-if="step === 'verify'" class="bg-indigo-600 p-4 md:p-3 pb-5 md:pb-4 text-white flex justify-between items-center rounded-xl">
                <div class="text-sm md:text-xs leading-tight">
                    ...
                    <p class="font-bold text-base md:text-sm">
                        {{ track.danceStyle }}
                        <span v-if="track.subStyle && track.subStyle !== track.danceStyle" class="font-normal opacity-90">
                            ({{ track.subStyle }})
                        </span>
                        • {{ tempoLabel }}
                    </p>
