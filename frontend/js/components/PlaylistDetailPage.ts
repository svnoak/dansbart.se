...
export default {
  ...
  setup() {
    ...
    const isOwner = computed(() => {
      ...
      return playlist.value.user?.id === currentUser.value.id ||
             playlist.value.userId === currentUser.value.id;
    });
    ...
    const canEdit = computed(() => {
      ...
      const userCollab = collaborators.find(c =>
        c.userId === currentUser.value.id && c.status === 'accepted'
      );
      return userCollab?.permission === 'edit';
    });

    const playlistIdStr = computed(() => {
      const id = playlistId.value;
      return Array.isArray(id) ? id[0] : id;
    });

    // Fetch playlist when route changes
    watch(playlistIdStr, async (newId) => {
      if (newId) {
    ...
    onMounted(async () => {
      if (playlistIdStr.value) {
        await fetchPlaylist(playlistIdStr.value);
      }
    });
