...
export default defineComponent({
  ...
  props: {
    artist: {
      type: Object as PropType<ArtistCardProps['artist']>,
      required: true,
    },
  },
  ...
  setup(props, { emit }) {
    const navigate = (): void => {
      emit('navigate-to-artist', props.artist.id);
    };
    return { navigate };
  },
  template: `
      ...
      <div class="flex items-center gap-4">
        ...
        <div class="flex-1 min-w-0">
          <div class="flex items-center gap-2 mb-1">
            ...
            <svg
              v-if="artist.is_verified ?? artist.isVerified"
          ...
          <p class="text-gray-600 text-sm">
            {{ (artist as { total_tracks?: number }).total_tracks ?? 0 }} {{ ((artist as { total_tracks?: number }).total_tracks === 1) ? 'låt' : 'låtar' }}
          </p>
        </div>
