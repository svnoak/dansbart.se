...
export default defineComponent({
  name: 'CompactTrackCard',
  props: {
    track: {
      type: Object as PropType<TrackDisplay>,
      required: true,
    },
    currentTrack: {
      type: Object as PropType<TrackDisplay | null>,
      default: null,
    },
    isSpotifyMode: {
      type: Boolean,
  ...
  computed: {
    ...
    artistNames(): string {
      return this.track.artists?.map((a) => a.name).filter(Boolean).join(', ') || 'Okänd artist';
    },
    hasValidStyle(): boolean {
      return !!(this.track.danceStyle && this.track.danceStyle !== 'Unclassified');
    },
  },
    ...
    >
      ...
      <div v-if="hasValidStyle" class="flex-shrink-0">
        <span
          ...
          :title="track.subStyle ? track.danceStyle + ' › ' + track.subStyle : track.danceStyle"
        >
          {{ track.danceStyle }}
        </span>
      </div>
