...
export default defineComponent({
  ...
  methods: {
    onConsentChanged(event: CustomEvent<{ status: string }>) {
    ...
    createPlayer() {
      ...
      this.ytPlayer = new window.YT.Player('hidden-yt-player', {
        height: '100%',
        width: '100%',
        ...
        events: {
          ...
          onStateChange: e => {
            ...
            if (e.data === window.YT.PlayerState.CUED && this.shouldAutoplay) {
              this.shouldAutoplay = false;
              e.target.playVideo();
            }

            if (e.data === window.YT.PlayerState.PLAYING) this.startTimer();
            else this.stopTimer();

            if (e.data === window.YT.PlayerState.ENDED) this.$emit('next');
          },
