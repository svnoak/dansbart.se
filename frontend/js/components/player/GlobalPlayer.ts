...
export default {
  ...
  watch: {
    isPlaying(val) {
      ...
      const activeSource = this.activeSource as string;
      if (activeSource === 'youtube' && this.$refs.ytEngine && !this.isLoadingVideo) {
        val ? this.$refs.ytEngine.play() : this.$refs.ytEngine.pause();
      }
      if (activeSource === 'spotify' && this.$refs.spotifyEngine) {
  ...
  computed: {
    spotifyTrackId() {
      if (!this.currentTrack?.playbackLinks) return null;
      // Use platform field to find Spotify link
      let link = this.currentTrack.playbackLinks.find(l => l.platform === 'spotify');
      if (!link) {
        // Fallback: check for old URL format
        link = this.currentTrack.playbackLinks.find(l => {
    ...
    trackArtist() {
      ...
      if (typeof t.artistName === 'string' && t.artistName) return t.artistName;
    ...
    async fetchVersions(trackId) {
      ...
      try {
        ...
        if (res.ok) {
          ...
          const activeIdx = this.availableVersions.findIndex(v => v.isActive);
