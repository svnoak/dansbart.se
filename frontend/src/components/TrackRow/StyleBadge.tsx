...
export function StyleBadge({
  ...
  if (!hasValidStyle) {
    badgeStyle = {};
    badgeClasses =
      'rounded-full border border-[rgb(var(--color-border))] bg-[rgb(var(--color-pill-bg))] px-2 py-0.5 text-sm font-bold uppercase tracking-wide text-[rgb(var(--color-text-muted))]';
  } else if (isVerified || isAiHigh) {
    badgeStyle = { backgroundColor: bgColor, color: textColor };
    badgeClasses =
      'inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-sm font-bold uppercase tracking-wide transition-opacity hover:opacity-80';
  } else {
    ...
    badgeClasses =
      'inline-flex items-center gap-1 rounded-full border border-dashed bg-[rgb(var(--color-bg-elevated))] px-2 py-0.5 text-sm font-bold uppercase tracking-wide transition-opacity hover:opacity-80';
  }
