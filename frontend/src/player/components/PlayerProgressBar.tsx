    ...
    return (
      <div className="relative w-full h-8 flex flex-col justify-end">
        ...
        <div className="absolute top-0 left-0 right-0 h-4 pointer-events-none select-none">
          ...
          <span
            className="absolute text-xs font-mono text-[rgb(var(--color-text-muted))]/60"
            style={{ left: '0%' }}
          >
            1
          </span>
          {barTicks.map((tick, i) => {
            ...
            return shouldShowLabel(barNum) ? (
              <span
                key={i}
                className="absolute text-xs font-mono text-[rgb(var(--color-text-muted))]/60 -translate-x-1/2"
        ...
        >
          ...
          {hoveredBar !== null && (
            <div
              className="absolute -top-7 -translate-x-1/2 z-20 pointer-events-none bg-[rgb(var(--color-bg-elevated))] border border-[rgb(var(--color-border))] rounded px-1.5 py-0.5 text-xs font-mono text-[rgb(var(--color-text))] shadow whitespace-nowrap"
